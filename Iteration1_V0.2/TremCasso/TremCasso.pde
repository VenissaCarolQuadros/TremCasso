...
void setup(){
  ...
  positions =new float[] {4.5, 9.75};
  drawButtons(positions);
  drawGUI();
...
public void YforceSetter(){
  if (click){
    ...
    f_ee.set(f_ee.x+2.5, f_ee.y);
  }
  if (buttonForces!=0)
  {
    if (buttonForces==2)
    {
        f_ee.set(f_ee.x, f_ee.y+20);
    }
    else
    {
        f_ee.set(f_ee.x, f_ee.y+10);
    }
       
  }
}

public boolean checkPosGetDelay(int index){
  //print(s.h_avatar.getY(), "  ", positions[1]+2.75, "\n");
  int delayGrad;
  ...
  if (millis()-counters[index]>delayGrad){
    counters= new float[]{millis(), millis(), millis(), millis()};
    click=true;
    return true;
  }
